...
val canonicalVersionCode = 260727 // alias: 1719
val canonicalVersionName = "26.07.27" // alias: "8.19.2"
val currentHotfixVersion = 0
val maxHotfixVersions = 100
val grapheneRevision = 3

// GRAPHENE: Use default debug keystore generated by Android Studio at $HOME/.android/debug.keystore
...
android {
  ...
  testOptions {
    ...
    unitTests {
      ...
      // GRAPHENE: Reduce memory pressure by limiting the number of test classes per runner
      all { task ->
        task.forkEvery = 64
      }
    }
  ...
  packaging {
    jniLibs {
      ...
      // GRAPHENE: Compress native libs by default as APK is not split on ABIs
  ...
  if (grapheneRevision < 0 || currentHotfixVersion < 0 || (grapheneRevision + currentHotfixVersion) >= maxHotfixVersions) {
    throw GradleException("Graphene revision $grapheneRevision or Hotfix version $currentHotfixVersion out of range")
  }
  ...
  defaultConfig {
    val sourceVersionNameWithRevision = "${canonicalVersionName}-${grapheneRevision}"

    versionCode = (canonicalVersionCode * maxHotfixVersions) + grapheneRevision + currentHotfixVersion
    ...
    buildConfigField("String", "FDROID_UPDATE_URL", "\"https://github.com/Graphene-Lab/GrapheneMessenger/releases/latest/download\"")
  ...
  productFlavors {
    create("store") {
      ...
      buildConfigField("boolean", "MANAGE_GRAPHENE_UPDATES", "false")
    }

    create("website") {
      dimension = "distribution"
      buildConfigField("boolean", "MANAGE_GRAPHENE_UPDATES", "true")
      isDefault = true
    }
  ...
  lint {
    ...
    if (ciEnabled) {
      // GRAPHENE: disable slow "newer dependency available" lint checks; versions are pinned.
      disable += setOf(
        "GradleDependency",
...
dependencies {
  ...
  implementation(libs.graphene.argon2) { artifact { type = "aar" } }
  implementation(libs.graphene.native.utils)
