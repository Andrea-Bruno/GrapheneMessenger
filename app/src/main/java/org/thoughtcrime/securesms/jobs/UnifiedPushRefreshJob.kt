...
/**
 * Handles UnifiedPush registration and ensures the GrapheneSocket status is up-to-date.
 ...
 *   Used during first registration to a new GrapheneSocket server.
...
class UnifiedPushRefreshJob private constructor(
  ...
  public override fun onRun() {
    ...
    if (!enabled) {
      ...
      cleanupGrapheneSocketDevice()
      return
    }

    try {
      validateGrapheneSocketDevice()
      ...
      when (newStatus) {
        ...
        RegistrationStatus.FORBIDDEN_ENDPOINT -> UnifiedPushNotificationBuilder(context).setNotificationGrapheneSocketForbiddenEndpoint()
        RegistrationStatus.FORBIDDEN_UUID -> UnifiedPushNotificationBuilder(context).setNotificationGrapheneSocketForbiddenUuid()
        RegistrationStatus.FORBIDDEN_PASSWORD -> UnifiedPushNotificationBuilder(context).setNotificationGrapheneSocketForbiddenPassword()
        else -> {}
      }
    } catch (t: Throwable) {
  ...
  private fun checkRegistrationStatus(): RegistrationStatus {
    ...
    val grapheneSocketUrl = SignalStore.unifiedpush.grapheneSocketUrl?.toHttpUrlOrNull()
    ...
    if (!airGapped && grapheneSocketUrl == null) {
      Log.e(TAG, "Missing GrapheneSocket URL.")
      return RegistrationStatus.PENDING
    }

    val device = SignalStore.unifiedpush.device ?: try {
      GrapheneSocketRepository.createDevice().also { newDevice ->
        SignalStore.unifiedpush.device = newDevice
        SignalStore.unifiedpush.vapidKeySynced = true
        Log.d(TAG, "Created new GrapheneSocket device: $newDevice")
      }
    ...
    Log.d(TAG, "Re-registering $device on GrapheneSocket server...")

    val result = GrapheneSocketRepository.registerDeviceOnServer(
      url = grapheneSocketUrl!!,
      device = device,
      endpoint = endpoint,
      ping = testPing,
    )
  ...
  @Throws(IOException::class)
  private fun cleanupGrapheneSocketDevice() {
    if (SignalStore.unifiedpush.airGapped) {
      Log.d(TAG, "GrapheneSocket is air-gapped, cleanup skipped.")
      return
    }
  ...
  @Throws(IOException::class)
  private fun removeAndClearDevice(device: GrapheneSocketDevice) {
    GrapheneSocketRepository.removeDevice(device)
    clearDevice()
  }
