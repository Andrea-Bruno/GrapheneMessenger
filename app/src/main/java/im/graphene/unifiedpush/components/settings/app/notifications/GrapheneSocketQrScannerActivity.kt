...
/**
 * Prompts the user to scan a GrapheneSocket QR code. Uses the activity result to communicate the recipient that was found, or null if no valid deeplink were scanned.
 * See [Contract].
 */
class GrapheneSocketQrScannerActivity : AppCompatActivity() {

  private val viewModel: GrapheneSocketQrScannerViewModel by viewModels()
  ...
  class Contract : ActivityResultContract<Unit, GrapheneSocket?>() {
    override fun createIntent(context: Context, input: Unit): Intent {
      return Intent(context, GrapheneSocketQrScannerActivity::class.java)
    }

    override fun parseResult(resultCode: Int, intent: Intent?): GrapheneSocket? {
      return if (resultCode == RESULT_OK) {
        GrapheneSocket.parseLink(intent?.data!!)
      } else {
        null
      }
    }
...
fun Content(
  ...
  state: GrapheneSocketQrScannerViewModel.ScannerState,
