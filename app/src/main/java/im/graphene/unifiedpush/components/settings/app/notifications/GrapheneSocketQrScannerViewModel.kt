...
class GrapheneSocketQrScannerViewModel : ViewModel() {
  ...
  fun onQrScanned(data: String) {
    ...
    disposables += GrapheneSocketQrScanRepository.lookupQrLink(data)
  ...
  fun onQrImageSelected(context: Context, uri: Uri) {
    ...
    disposables += GrapheneSocketQrScanRepository.scanImageUriForQrCode(context, uri)
      .observeOn(AndroidSchedulers.mainThread())
      .subscribeBy { result ->
        _state.value = _state.value.copy(
          qrScanResult = result,
