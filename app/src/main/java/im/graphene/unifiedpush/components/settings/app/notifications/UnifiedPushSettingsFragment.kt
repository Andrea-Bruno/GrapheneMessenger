...
class UnifiedPushSettingsFragment : DSLSettingsFragment(R.string.NotificationDeliveryMethod__unifiedpush) {
  ...
  private val qrScanLauncher: ActivityResultLauncher<Unit> =
    registerForActivityResult(GrapheneSocketQrScannerActivity.Contract()) { grapheneSocket ->
      if (grapheneSocket != null) {
        viewModel.setGrapheneSocket(grapheneSocket)
      }
    }
  ...
  private fun getConfiguration(state: UnifiedPushSettingsState): DSLConfiguration {
    return configure {
      ...
      clickPref(
        title = DSLSettingsText.from(R.string.UnifiedPushSettingsFragment__graphenesocket_server),
        summary = DSLSettingsText.from(
          if (state.airGapped) {
            getString(R.string.UnifiedPushSettingsFragment__graphenesocket_server_air_gapped)
          } else {
            state.grapheneSocketUrl ?: "Error"
          }
        ),
        onClick = {
      ...
      if (state.airGapped) {
        ...
        clickPref(
          ...
          summary = DSLSettingsText.from(getString(R.string.UnifiedPushSettingsFragment__tap_to_request_a_test_notification_from_graphenesocket)),
          onClick = {
            viewModel.pingGrapheneSocket()
  ...
  private fun getStatusSummary(state: UnifiedPushSettingsState): Int {
    return when {
      ...
      state.grapheneSocketUrl == null && !state.airGapped -> R.string.UnifiedPushSettingsFragment__status_summary_graphenesocket_url_missing
