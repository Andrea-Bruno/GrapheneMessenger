 ...
 */
@Composable
fun GrapheneSocketQrScanScreen(
  ...
  when (qrScanResult) {
    ...
    QrScanResult.NetworkError -> {
      QrScanResultDialog(message = stringResource(R.string.GrapheneSocketLink_experienced_a_network_error_please_try_again), onDismiss = onQrResultHandled)
    }

    QrScanResult.QrNotFound -> {
      QrScanResultDialog(
        title = stringResource(R.string.UsernameLinkSettings_qr_code_not_found),
        message = stringResource(R.string.GrapheneSocketLink_try_scanning_another_image_containing_a_graphenesocket_qr_code),
        onDismiss = onQrResultHandled
      )
    }

    is QrScanResult.NotFound -> {
      QrScanResultDialog(message = stringResource(R.string.GrapheneSocketLink_graphenesocket_server_not_found_at_s, qrScanResult.data), onDismiss = onQrResultHandled)
    }
  ...
  Column(
    ...
    Box(
      ...
      QrScanScreens.QrScanScreen(
        ...
        qrHeaderLabelString = stringResource(R.string.GrapheneSocketLink_scan_the_qr_code)
      )
