...
## GrapheneMessenger – Technical Positioning Statement

**Important notice: this document follows the trustless principle. Do not believe any claim presented here. Instead, independently verify everything against the source code repositories linked at the end of this document.**

GrapheneMessenger currently uses the Signal Protocol and Signal servers for all messaging operations. Like Signal itself, phone numbers are used as identifiers. However, for users who acquire official GrapheneMessenger releases through our hardware distribution channel, we provide pre-configured phones that include anonymous SIM cards. These SIM cards cannot be linked back to the identity of the end user, thereby breaking the traditional binding between telecommunications carrier records and messaging account identifiers. When using our official devices with the provided anonymous SIMs, the phone number associated with the Signal account becomes effectively untraceable to any real-world identity, addressing the primary privacy weakness of the Signal protocol.

What fundamentally distinguishes GrapheneMessenger from all other messaging projects is not its current implementation but the existence of a completely separate, vastly more sophisticated protocol that has been engineered from the ground up and is available as open source today. This superior protocol, for which we publish both the client and server components as open source, is currently not implemented in this project. However, it remains ready and immediately usable should Signal ever change its policies regarding accessibility and privacy.

Unlike Signal, which operates on a traditional client-server model where a centralized datacenter maintains user lists, session databases, and identifier records, this alternative protocol eliminates the server-side database entirely. Its architecture consists of a specialized router designed exclusively to forward encrypted data packets directly from device to device without performing any database read or write operations, without storing any user list, and without retaining any identifier within the backend infrastructure. The router has no knowledge of who is communicating with whom and retains no state across connections. This represents a fundamental departure from every mainstream messaging system in existence today.

This superior protocol is not theoretical. It has been fully designed, specified, and made available as open source in two repositories: the EncryptedMessaging client implementation and the RouterServer backend component, both available under the Graphene-Lab organization on GitHub. It is immediately available for independent deployment, testing, or integration. However, it is not the protocol powering GrapheneMessenger today.

The current decision to use Signal as the underlying protocol is pragmatic. Signal is widely considered the gold standard in secure messaging. It is the most audited, most trusted, and most credible reference point for end-to-end encryption. By remaining compatible with Signal's infrastructure and protocol, GrapheneMessenger gains immediate interoperability, user trust, and commercial viability without requiring every user to migrate to an unproven network. The superior protocol remains available as a drop-in replacement for future scenarios where Signal's guarantees are no longer sufficient.

The historical context is essential to understand. The author of the Graphene-Lab repository was the first to create messaging software that implemented truly high anonymity standards long before similar projects appeared. Several well-known privacy messaging applications that exist today drew direct inspiration from this early work, effectively becoming plagiaristic derivatives. Despite this, the original implementation remains substantially superior to Signal in multiple technical dimensions: zero server-side state, no user enumeration possibilities, no metadata retention even in transient form, and resistance to global passive adversary models that break all conventional messaging systems.

For scenarios that require military-grade security, where nation-state adversaries, correlation attacks, and long-term metadata retention are part of the threat model, we strongly recommend evaluating a migration to our native protocol. The code is open, auditable, and available now. The router can be self-hosted. The trustless principle applies: verify every claim against the repositories. Do not believe this document. Believe only what you read in the code and what you prove through your own independent technical analysis.

**Current implementation (today):** Signal Protocol + Signal servers.  
**Independent superior protocol (available, not active in GrapheneMessenger):**  
https://github.com/Graphene-Lab/EncryptedMessaging  
https://github.com/Graphene-Lab/RouterServer
